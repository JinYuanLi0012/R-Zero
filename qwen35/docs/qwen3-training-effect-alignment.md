...
| Solver boxed 提取与投票 | 固定采样 n/m 次；无 box 的 MathRuler 字符串 `None` 保留，显式空 `\\boxed{}` 被删除 | 曾用自写 extractor 将两者都变为空字符串 | 否 | **直接使用 MathRuler extractor** | 离线难度分母是非空提取数，可合法出现 1/8、1/7；多数 `None` 后续被过滤 | sentinel、空 box 与调用点回归测试 |
