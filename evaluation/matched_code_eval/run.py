"""Shared code instructions with family-specific training templates for Base and Solver."""
import argparse
import csv
...
from evaluation.matched_code_eval.protocol import SYSTEM, template_for, configure, render, TokenInputModel


def generate(args):
    from transformers import AutoConfig, AutoTokenizer
    if AutoConfig.from_pretrained(args.model, revision=args.revision).model_type != {"octo": "llama", "qwen": "qwen3"}[args.family]:
        raise ValueError("Model architecture does not match --family")
    ...
    for dataset in args.datasets:
        ...
        tasks = [render(t, tokenizer, args.max_new_tokens, args.max_model_len, args.family)
...
def parse_args():
    ...
    parser.add_argument("--model", required=True, help="Complete merged Solver checkpoint or original family Base")
    parser.add_argument("--base-model", action="store_true", help="Mark original Base in metadata; input processing is identical to Solver")
...
def main():
    args = parse_args()
    template = template_for(args.family)
    protocol = {"name": args.family + "-matched-code-v1", "template": template,
                "template_hash": digest(template), "input_kind": "prompt_token_ids",
                "add_special_tokens": False, "prompt_style": "chat",
                "system_instruction": SYSTEM, "thinking_override": None,
                "generation_config": "vllm", "temperature": 0.0, "samples_per_task": 1}
    config = {k: getattr(args, k) for k in ["family", "revision", "base_model", "datasets", "max_new_tokens",
    ...
    config.update({"model": model_identity(args.model, args.revision), "protocol": protocol,
                   ...
                   "implementation": digest({str(p.relative_to(ROOT)): p.read_text() for p in
                       [*sorted(HERE.glob("*.py")), *sorted(LEGACY.glob("*.py")),
                        ROOT / "evaluation/octo_code_eval/protocol.py"]})})
    ...
    with (args.output / ".lock").open("w") as lock:
        ...
        if args.stage in {"all", "score"}:
            ...
            if not (args.output / "tokenizer_protocol.json").is_file():
                raise ValueError("Missing tokenizer protocol; run the matched generation stage first")
            for dataset in args.datasets:
