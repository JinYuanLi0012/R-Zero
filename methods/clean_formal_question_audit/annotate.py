#!/usr/bin/env python3
"""Run blind validity/answer passes, then judge source majority answers."""
...
from annotate import (
    ANSWER_PROMPT_VERSION, ANSWER_SYSTEM_PROMPT,
    VALIDITY_PROMPT_VERSION, VALIDITY_SYSTEM_PROMPT,
)
from batch_annotate import run_batch_pass, summarize_batch_state
...
def main() -> None:
    ...
    if any(blind_by_id[item_id]["question"] != row["question"] for item_id, row in sampled_by_id.items()):
    ...
    validity = run_batch_pass(
        client, blind, "validity", args.output_dir, args.model, args.reasoning_effort,
        args.max_output_tokens, args.max_attempts, args.min_confidence, args.poll_seconds,
    )
    valid_items = [
        blind_by_id[row["id"]] for row in validity
    ...
    for row in answers:
        ...
        if (
            row["status"] == "complete" and result.get("answer_verified") is True
            ...
            majority_inputs.append({
                "id": row["id"], "question": blind_by_id[row["id"]]["question"],
    ...
    majority_results = run_majority_pass(
        client, majority_inputs, args.output_dir, args.model, args.reasoning_effort,
        args.max_output_tokens, args.max_attempts, args.min_confidence, args.poll_seconds,
    )
    ...
    atomic_json(args.output_dir / "annotation_manifest.json", {
        ...
        "max_attempts": args.max_attempts, "min_confidence": args.min_confidence,
        "input_count": len(blind), "answer_pass_input_count": len(valid_items),
        ...
        "majority_batches": summarize_batch_state(args.output_dir, "majority"),
    })
    print(
        f"[annotate] complete: validity={len(validity)} answer={len(answers)} "
