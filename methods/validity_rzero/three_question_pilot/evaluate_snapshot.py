...
for index, (golden_answer, question) in enumerate(zip(answers, questions)):
    try:
        ...
        if gate is not None and gate["validity_decision"] == "INVALID":
            ...
            item.update({key: correct_data[index][key] for key in ("sample_id", "request_id", "question_position", "shard", "request_index") if key in correct_data[index]})  # PILOT_PROVENANCE_ONLY
