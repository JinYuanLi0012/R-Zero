#!/usr/bin/env python3
"""Score generative-v3/vLLM semantic-pair outputs as a diagnostic rerun."""
...
from run_pair_judge_v3_vllm import parse_response_v3
...
def validate_and_join(
    ...
    for prediction in prediction_rows:
        ...
        reparsed = parse_response_v3(str(prediction["raw_response"]))
        for field in (
            "predicted_label", "parsed_label", "format_status", "format_error_reason",
        ):
            if prediction.get(field) != reparsed[field]:
                raise ValueError(
                    f"saved parse disagrees with strict v3 parser for {pair_id} "
...
def classification_metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    parseable = [row for row in rows if row["predicted_label"] != "FORMAT_ERROR"]
    parseable_correct = sum(row["correct"] for row in parseable)
    ...
    return {
        "count": len(rows),
        "correct": correct,
        "accuracy_end_to_end": safe_rate(correct, len(rows)),
        "parseable_count": len(parseable),
        "parseable_correct": parseable_correct,
        "accuracy_parseable_only": safe_rate(parseable_correct, len(parseable)),
        "confusion_matrix": {
            f"gold_{gold}": {
...
def analyze(
    ...
    for pair_id in pair_ids:
        ...
        if "FORMAT_ERROR" in (first_label, second_label):
            format_error_ids.append(pair_id)
        else:
            both_parseable += 1
    ...
    return {
        ...
        "order_stability": {
            ...
            "pairs_with_any_format_error": len(format_error_ids),
            "format_error_pair_ids": format_error_ids,
        },
        "error_condition_count": len(errors),
        "pairs_with_any_error": sorted({row["pair_id"] for row in errors}),
        "interpretation": (
            "End-to-end accuracy is primary. Parseable-only accuracy is diagnostic "
            "and cannot replace it. This is the same 50-pair diagnostic rerun, not "
            "a new held-out validation."
        ),
...
def make_report(
    ...
    lines = [
        "# Generative-v3/vLLM semantic judge: diagnostic rerun", "",
        "This reruns the same 50 pairs. It is not a new held-out validation.", "",
        "End-to-end accuracy is primary; parseable-only accuracy is diagnostic only.", "",
        "## Metrics by question order", "",
        "| Order | End-to-end accuracy | Parseable-only accuracy | FN | FP | Format errors |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    for order in ORDERS:
        row = metrics["conditions"][order]
        parseable_accuracy = row["accuracy_parseable_only"]
        parseable_text = "n/a" if parseable_accuracy is None else f"{parseable_accuracy:.1%}"
        lines.append(
            f"| `{order}` | {row['correct']}/{row['count']} "
            f"({row['accuracy_end_to_end']:.1%}) | {row['parseable_correct']}/"
            f"{row['parseable_count']} ({parseable_text}) | {row['false_negatives']} | "
            f"{row['false_positives']} | {row['format_errors']} |"
        )
    ...
    for number, row in enumerate(errors, 1):
        lines.extend([
            ...
            f"Finish reason: `{row.get('finish_reason')}`; prompt/output tokens: "
            f"`{row.get('prompt_token_count')}/{row.get('output_token_count')}`.", "",
...
def main() -> None:
    ...
    atomic_json(args.output_dir / "metrics_v3.json", metrics)
    atomic_jsonl(args.output_dir / "errors_v3.jsonl", errors)
    atomic_jsonl(args.output_dir / "order_disagreements_v3.jsonl", disagreements)
    (args.output_dir / "report_v3.md").write_text(
        make_report(metrics, errors, disagreements), encoding="utf-8",
    )
    print(json.dumps(metrics, indent=2, sort_keys=True))
    print(f"wrote generative-v3 diagnostic report to {args.output_dir}")
