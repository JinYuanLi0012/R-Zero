#!/usr/bin/env python3
"""Run the v6 exercise-pattern brief-reasoning diagnostic."""
...
def main() -> None:
    ...
    prediction_path = args.output_dir / "predictions_v6_pattern_reasoning.jsonl"
    manifest_path = args.output_dir / "run_manifest_v6_pattern_reasoning.json"
    if not args.overwrite and (prediction_path.exists() or manifest_path.exists()):
        raise FileExistsError("v6 reasoning output exists; use a new directory or --overwrite")
    ...
    manifest = {
        "experiment": "semantic_judge_offline_50_v6_pattern_brief_reasoning",
        "evaluation_status": "diagnostic_prompt_test_not_held_out_validation",
        "controlled_baseline": "v5_exercise_pattern_direct_label",
        "only_intended_variable": "brief reasoning before the final boxed label",
        "prompt_version": PROMPT_VERSION,
    ...
    print(f"wrote {len(predictions)} v6 reasoning conditions to {prediction_path}")
