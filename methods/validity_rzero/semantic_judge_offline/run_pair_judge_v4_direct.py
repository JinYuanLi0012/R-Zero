#!/usr/bin/env python3
"""Run the controlled direct-label semantic-judge ablation with batched vLLM."""
...
PROMPT_VERSION = "semantic-pair-v4-direct-label-ablation"
PROMPT_TEMPLATE = (
...
def main() -> None:
    ...
    prediction_path = args.output_dir / "predictions_v4_direct.jsonl"
    manifest_path = args.output_dir / "run_manifest_v4_direct.json"
    if not args.overwrite and (prediction_path.exists() or manifest_path.exists()):
        raise FileExistsError("v4 direct output exists; use a new directory or --overwrite")
    ...
    manifest = {
        "experiment": "semantic_judge_offline_50_v4_direct_label_ablation",
        "evaluation_status": "diagnostic_ablation_not_held_out_validation",
        "controlled_baseline": "generative_v3_vllm_max1024",
        "only_intended_variable": "direct boxed label instead of brief analysis then label",
        "prompt_version": PROMPT_VERSION,
    ...
    print(f"wrote {len(predictions)} v4 direct-label conditions to {prediction_path}")
