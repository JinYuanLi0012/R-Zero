...
FINGERPRINT=$(python3 "$METHOD_DIR/pipeline_state.py" init \
  ...
  --field "solver_rollout_n=$SOLVER_ROLLOUT_N" \
  --field "solver_global_batch_size=$SOLVER_GLOBAL_BATCH_SIZE" \
  --field "center_rollout_tensor_parallel_size=$CENTER_ROLLOUT_TENSOR_PARALLEL_SIZE" \
