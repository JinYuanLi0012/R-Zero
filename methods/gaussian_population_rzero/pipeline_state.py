#!/usr/bin/env python3
"""Atomic stage state for the isolated Gaussian-population experiment."""
...
def init_state(path: Path, fields: dict[str, str]) -> str:
    ...
    if path.exists():
        ...
        if state.get("run_fingerprint") != signature:
            raise RuntimeError("run configuration changed; use a new RUN_NAME")
    else:
        _atomic_json(
            path,
            {
...
def main() -> int:
    ...
    if args.command == "init":
        print(init_state(args.state, _parse_pairs(args.field)))
        return 0
